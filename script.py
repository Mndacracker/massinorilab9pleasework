...
from poki__api import get_pokemon_info


# Creating the window
root = Tk()
root.title("Pokemon lab9")
...
frm_top.grid(row=0, column=0, columns=2, padx=(20, 10))
...
def manage():
    # Entering Pokemon name by the user
    poke_name = ent_name.get().strip()
    if len(poke_name) == 0:
    
    # This is where you add a pokemon name 
        return
    
    # This is where the pokemon information is
    poke_info = get_pokemon_info(poke_name)
    if poke_info is None:
        err_msg = f'Information not patched {poke_name.capitalize()} from the PokeAPI.'
